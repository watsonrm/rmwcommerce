...
2. For any doc that needs the footer, use the [Drive `files.copy`](https://developers.google.com/workspace/drive/api/reference/rest/v3/files/copy) call (the MCP server typically exposes this as `copyFile`) to clone the template, then overlay your body content with `replaceDocumentWithMarkdown`. Most MCP server implementations expose a `firstHeadingAsTitle: false` option on the markdown overlay — set it so the body H1 stays a `HEADING_1` rather than getting promoted to the doc title (which would interact badly with the template's title styling). The footer survives because it lives at the `documentStyle` level, not in the body stream that the markdown replacement touches.
