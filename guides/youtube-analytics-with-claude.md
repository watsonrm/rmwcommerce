...
**Corrections from prior circulating versions:** the Analytics/Reporting split is covered at [The three surfaces](#the-three-surfaces) and [Which surface do you need?](#which-surface-do-you-need); the not-Studio-only correction at [Path 2](#path-2); the creator-revenue block at [The traps](#the-traps); the no-service-account constraint and BigQuery resolution at [The auth trap](#auth) and [Path 4](#path-4).
