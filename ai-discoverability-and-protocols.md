...
**The alphabet soup of agent-facing formats — robots.txt, llms.txt, ai-plugin.json, openapi.yaml, MCP, WebMCP, A2A — separated by what you can deploy today, what you should watch, and what you should ignore.**
...
The discoverability and protocol landscape looks fragmented but is actually consolidating around three load-bearing standards: **OpenAPI for tool specs**, **MCP for tool and data connections**, and **llms.txt-style content discovery**. Meanwhile, ChatGPT-era formats are fading, browser-side surfaces are still speculative, and inter-agent protocols are competing without a clear winner.

Concrete recommendation: deploy `openapi.yaml` if you have a public API, deploy an MCP server if you want AI agents to call your service, configure `robots.txt` correctly for known agent user-agents, and add `llms.txt` if you have curated long-form content. Skip `ai-plugin.json`. Watch A2A and WebMCP without deploying them yet.

- Reduce wasted time evaluating and deploying formats that are already deprecated or not yet stable
- Know the one or two actions that will cover 80% of your agent-discoverability exposure today
...
| :-- | :--- | :--- | :--- |
| **1** | `openapi.yaml` | Tier 1 — do it | Load-bearing. Every agent ecosystem reads it. Already necessary for any public API. |
| **2** | MCP server | Tier 1 — do it | Actually winning for tool/data connections. Growing ecosystem. SDK support in Python and TypeScript. |
| **3** | `robots.txt` (agent user-agents) | Tier 1 — fix it | Plumbing that's wrong more often than not. Cheap to fix, meaningful for agent discovery. |
| **4** | `llms.txt` | Tier 2 — pragmatic adopt | Low cost, modest upside. Deploy if you have curated content. Don't over-engineer. |
| **5** | WebMCP | Tier 2 — track | Browser-side surface for in-page AI tools. Spec in progress. Not production-ready. |
| **6** | A2A | Tier 3 — read, don't deploy | Cross-vendor agent interop. Interesting, immature. 150+ org supporters but limited production deployments as of 2026. |
| **7** | `/.well-known/ai-plugin.json` | Tier 4 — skip | Deprecated. ChatGPT Plugins killed its primary consumer in April 2024. |

Most readers should handle rows 1–3 and check back on rows 4–5 in six months.

---

## How to use this guide

If you're a developer or technical architect: read straight through. The implementation notes for each format are in each section's "What to do" block.
...
- OpenAI launched and then effectively killed ChatGPT Plugins, leaving `ai-plugin.json` without a primary consumer.
- Anthropic launched MCP in November 2024 and donated it to the Linux Foundation (via the Agentic AI Foundation) in December 2025. It grew from a single-vendor protocol to 10,000+ servers and 97 million monthly SDK downloads in roughly one year.
...
- Jeremy Howard proposed `llms.txt` in September 2024. Adoption is real (Anthropic, Cloudflare, Vercel all deploy it) but no major LLM provider has made a binding commitment to consume it as canonical.
...
The Stripe technical field guide notes that providing explicit technical instructions for agents — including how to call your APIs — is one of the four foundations of agentic commerce readiness. ([source](https://stripe.com/guides/how-to-prepare-for-agentic-commerce-technical-field-guide)) The OpenAPI spec is the standardized mechanism for that.

### What's hype

There's no meaningful hype around OpenAPI itself — it's been a production standard for a decade. The only critique is legitimate: it's verbose, especially for simple APIs, and YAML indentation errors are a recurring frustration. Neither of those is a reason not to maintain a spec.
...
Make the spec URL public and stable. Agents discovering your API need to be able to fetch the spec without authentication. If your spec requires authentication to read, agents can't use it for discovery.
...
The wrong move is a blanket `User-agent: * Disallow: /` or blocking all AI crawlers in bulk. If you block GPTBot and ClaudeBot, you lose discovery in those agents' responses. Agents will route to competitors who are accessible. The right move is per-agent rules with rate limiting at the WAF level, not blocking.
...
The problem it's solving is real: LLMs trying to understand a website face a mismatch between context windows and website complexity. A full website's worth of HTML, navigation, ads, and JavaScript is both large and hard to parse. `llms.txt` gives LLM consumers a curated markdown path — a prioritized list of what's worth reading, in a format that's easier to consume than raw HTML.
...
If your site has long-form guides, documentation, or product information that agents might need to reason about, `llms.txt` gives them a curated path to that content rather than asking them to parse your full HTML. For sites with dense, structured content — like a product catalog or a technical documentation site — the reduction in parsing noise has real value.

It's one of the four discovery files the Stripe technical field guide names as part of agent-readiness infrastructure. ([source](https://stripe.com/guides/how-to-prepare-for-agentic-commerce-technical-field-guide))

### What's hype

"llms.txt is the new SEO" is the overclaim that circulates. Several accurate reductions: this is a proposal, not a ratified standard. No major LLM provider has made a binding public commitment to treat it as canonical input at inference time (as of the research date of this article — verify current state). Adoption by LLM providers is uneven. The format's impact on actual agent recommendations is not yet measurable in any publicly available data.

There's also a valid skeptical framing: one Medium piece describes it as "a dud" given the lack of explicit LLM provider commitment. That may be premature — adoption is growing — but the honest answer is that the leverage here is moderate and uncertain, not transformative.
...
Don't over-engineer. The file is a signal, not a guarantee. Treat it as infrastructure maintenance, not a strategic initiative.
...
ChatGPT Plugins were effectively deprecated in April 2024 when OpenAI shifted users to Custom GPTs. The manifest format lost its primary consumer. The `ai-plugin.json` path exists in many sites' well-known directories as a legacy artifact.

Some agent ecosystems still read it as a discovery hint, which is why Stripe's field guide mentions it as one of the files agents currently read. ([source](https://stripe.com/guides/how-to-prepare-for-agentic-commerce-technical-field-guide)) That describes current agent behavior — legacy agents checking known locations — not endorsement of the format's longevity.

### What's in it for you in 2026

Very little. The format is not load-bearing in any current agent ecosystem at meaningful scale.

### What to do

Ignore unless you are actively maintaining a legacy ChatGPT Plugin integration. If you are, migrate: the path forward is either a Custom GPT (for ChatGPT-specific use cases) or an MCP server (for broader agentic ecosystem exposure). The `ai-plugin.json` format is not the answer for either.
...
By mid-2026: over 10,000 active MCP servers, 97 million monthly SDK downloads, and first-class client support in Claude Desktop, Claude Code, ChatGPT, Cursor, Gemini, Microsoft Copilot, and Visual Studio Code.
...
The speed-of-adoption claim overstates where things stand. Despite strong numbers, MCP is primarily Anthropic-ecosystem-centric for production deployments as of mid-2026. OpenAI has begun supporting MCP, but the OpenAI ecosystem's primary integration pattern is still direct function calling. "Everyone's doing MCP" describes momentum, not universality.

The other overclaim: treating MCP as a replacement for proper API design. MCP is a discovery and invocation layer. It doesn't replace the need for well-designed endpoints, proper auth, and rate limiting. Those remain your infrastructure's job.
...
If you're a Claude Code user deploying agents internally: MCP servers for your own services (CRM, calendar, databases) are the infrastructure that makes Rung 3 and Rung 4 agents practical. The permissions guide covers what to allow at the project and user level.
...
1. **W3C WebMCP** — a spec being incubated by the W3C Web Machine Learning Community Group at `webmachinelearning.github.io/webmcp/`. ([source](https://webmachinelearning.github.io/webmcp/)) This spec defines a `navigator.modelContext` JavaScript API that allows web pages to expose AI-callable tools — JavaScript functions with natural language descriptions and structured schemas. Web pages that implement it function as MCP servers running in client-side script instead of on a backend.
...
The motivation behind both: today's MCP works over stdio (local processes) or HTTP (backend servers). Neither is suitable for a webpage that wants to directly expose its own functionality to an in-page AI assistant. If you're building a webapp where the AI assistant should be able to interact with the page's own state — reading what's displayed, filling out forms, triggering in-page actions — neither server-side MCP nor function calling directly addresses this.

### Why it matters

The longer-term direction of agentic browsing points toward in-page AI assistants that can interact with what you're viewing — future Claude-in-page integrations, Comet, and similar products. WebMCP is the infrastructure path being developed for that use case.

The near-term relevance is limited. If you're building a conventional web application today, server-side MCP or standard function calling handles your agent integration needs. WebMCP becomes relevant when you're building an application specifically designed for in-page AI interaction.

### What's hype

"WebMCP will replace MCP." This isn't the right framing. They're complementary surfaces: WebMCP is the browser-side surface for what MCP handles server-side. You're more likely to have both than to choose between them as your ecosystem matures.

The naming confusion between the W3C spec and MCP-B is itself a signal of immaturity. When multiple incompatible proposals share a name, neither has won yet.
...
If you're building an application specifically for in-page AI agents as an experiment or early-adopter product, MCP-B provides an implementation path today with the explicit caveat that it may not align with the eventual W3C standard.
...
In June 2025, Google donated A2A to the Linux Foundation, establishing the Agent2Agent Protocol Project under neutral governance. Version 0.3 shipped in August 2025. As of mid-2026, A2A has 150+ organizational supporters including Atlassian, Salesforce, SAP, ServiceNow, and others — reflecting enterprise interest, though not yet widespread production deployment.

The relationship to MCP: A2A and MCP are complementary. MCP handles what agents can access from tools and data (the tool/resource connection layer). A2A handles how agents communicate with and delegate to each other (the agent coordination layer). You'd potentially use both in a system that has both internal tool access (MCP) and cross-vendor agent delegation (A2A).
...
For most current agent builds, the patterns in [Multi-Agent Fan-Out and Verification](multi-agent-fan-out-and-verification.md) — typed return contracts, thin orchestrators, subagent specialization — handle everything you need without A2A. The question of "when do I need A2A" has a concrete answer: when your agent needs to delegate to an agent you don't control, owned by a different organization with different auth.

### What's hype

Positioning A2A as "the standard for agent interop." There is no ratified standard. A2A is one well-supported proposal from a major vendor. The Agent Communication Protocol (ACP) and others exist and are in active development. "150+ supporters" reflects enterprise interest and Google's ecosystem influence; it doesn't indicate production adoption at scale.

The Linux Foundation donation in June 2025 — two months after the announcement — suggests Google moved quickly toward neutral governance. That's a good sign for long-term credibility, but the spec is still under active development (v0.3 in August 2025 is explicitly not v1.0).

### What to do

Read about it. Understand the architecture. Don't deploy in production unless you have a concrete cross-vendor agent-delegation use case you're actively trying to solve. If you do have that use case, evaluate A2A against alternatives (direct API integration, custom auth protocols) to confirm it's the right tool for your specific trust-boundary problem.

I haven't deployed A2A and don't plan to until I have a specific cross-vendor agent delegation problem that can't be solved cleanly by MCP plus direct API integration. That's the honest version of "Tier 3 — read about, don't deploy."
...
The term "function calling" is OpenAI's coinage. Anthropic calls the same capability "tool use." The mechanism is essentially identical across vendors: define tools as JSON schemas with names and parameter definitions; the model responds with a structured tool invocation; your code runs the tool and returns the result. That cycle is the ReAct loop (see [The Prompts-to-Agents Ladder](the-prompts-to-agents-ladder.md#react-loop-yao-et-al)) in its concrete API implementation.

Every major LLM API has this capability: OpenAI's chat completions, Anthropic's Messages API, Google's Gemini API, and others all implement it. It's the foundational capability that makes agents possible — without it, models can only produce text; with it, they can request actions.
...
Understanding this matters for avoiding architectural confusion: when you deploy an MCP server, you're not replacing function calling — you're adding a discovery and invocation layer on top of it. MCP tells agents what functions exist and what they do; function calling is the mechanism the agent uses to invoke them.
...
Use function calling (or tool use, in Anthropic terminology) directly when building single-agent workflows without a standardized discovery layer. Use MCP when you want that discovery and invocation surface shared across multiple agent consumers.
...
## Where this is converging

Three distinct things are happening simultaneously.

**Consolidating around load-bearing standards.** OpenAPI is the underlying spec format for describing what tools do — it's been load-bearing for a decade and that won't change. MCP is consolidating as the tool/data connection protocol within the Anthropic ecosystem and is growing broader; its Linux Foundation governance makes it a plausible cross-vendor standard over time. `llms.txt`-style content discovery is a real pattern with real adoption, even if the spec isn't yet ratified.

**Competing without a winner (agent-to-agent protocols).** A2A, ACP, and others are competing in the agent-coordination protocol space. This is the most genuinely fragmented area. The fact that both A2A and MCP ended up under Linux Foundation governance in late 2025 is a signal that neutral governance is the prerequisite for cross-vendor adoption, but it doesn't guarantee A2A specifically wins. Wait before deploying.

**Fragmenting in browser-side surfaces.** WebMCP has multiple competing proposals (W3C spec, MCP-B) with overlapping names. This will resolve — probably toward the W3C spec once browsers implement it — but not yet. Track, don't deploy.

**Already decided.** `ai-plugin.json` is done. ChatGPT Plugins are deprecated, the manifest format has no load-bearing consumer, and every current integration path leads away from it.

The practical implication: the decision tree is simple. For APIs: OpenAPI spec. For agent tool/data access: MCP server. For content discovery: robots.txt + optional llms.txt. For everything else: wait.

---

## Where to go next

**[RMW Commerce Glossary](glossary.md#discoverability-and-protocols)** — one-liner definitions for every term in this article, with hype vs. reality marked and cross-links to primary sources.
...
- Stripe — *How to prepare for agentic commerce: A technical field guide*. Cited for: the four discovery files agents read, robots.txt agent-user-agent guidance, llms.txt description, server-side rendering for agent token efficiency, agent fraud signal considerations. Verified HTTP 200: https://stripe.com/guides/how-to-prepare-for-agentic-commerce-technical-field-guide
...
- Model Context Protocol — specification and ecosystem stats. Verified (redirects to live spec): https://modelcontextprotocol.io/introduction
