...
*By [Rick Watson](https://rmwcommerce.com) · 2026-05-22 · Reference — skim by category*

Who this is for: readers of Rick's guides who want a one-stop definition of any named concept, with the hype call attached.
...
- **What's hype:** Reflection is one of four agentic design patterns Ng describes ([source](https://www.deeplearning.ai/the-batch/how-agents-can-improve-llm-performance/)). It adds latency and token cost. Production agents at scale often run without explicit reflection loops, relying instead on tool use and planning. Reach for reflection when the task has a verifiable success criterion the agent can check against. Skip for narrow tool-use tasks where quality is already verifiable by other means. Infinite reflection loops are the specific failure mode: reflection should be bounded to one or two passes or the cost does not justify the gain.
...
- **What's hype:** Plan mode separates "decide what to build" from "build it." Highest value on ambiguous multi-file changes where the implementation choices matter. For one-off edits with clear intent, plan mode adds overhead without proportional benefit — running it for tightly scoped, low-stakes operations is the wrong use. Worth the cost for complex multi-file work with real decision points; skip for clearly bounded tasks.
...
- **What's hype:** Some commentary positions `llms.txt` as "the new SEO." That framing predicts adoption that has not yet materialized. The spec is an Answer.AI proposal ([source](https://www.answer.ai/posts/2024-09-03-llmstxt.html)) — not ratified by a standards body. Adoption signals are uneven; this article found no public commitment from Anthropic, OpenAI, Google, or other major LLM providers to consume `llms.txt` as canonical input as of May 2026. Simon Willison — one of the most credible practitioners writing on LLMs — has implemented llms.txt-style tooling in his own developer documentation projects and called the concept "fantastic," but his engagement is with the developer-tooling use case, not general web discovery ([source](https://simonwillison.net/2025/Apr/14/llms-as-the-first-line-of-support/)). The leverage here is moderate and uncertain, not transformative.
...
### /.well-known/ai-plugin.json (wound down)
...
- **What's in it for you:** Nothing in 2026. OpenAI wound down the ChatGPT Plugins beta in phases: new plugin conversations ended March 19, 2024; all existing plugin chats shut down April 9, 2024. ([source](https://community.openai.com/t/plugin-store-and-new-chats-with-plugins-closed-march-19-2024/689877)) OpenAI's stated reason: "GPTs offer a better way to reach ChatGPT users." The `ai-plugin.json` manifest format lost its primary consumer at that point and has no load-bearing role in current agent ecosystems.
- **What's hype:** Danny Smith's field guide names `ai-plugin.json` as one of "four files agents read" — that list describes legacy agent behavior accurately, but this is the non-live member of the four. The other three are live; this one is not. Any claim that you need to create or maintain this file in 2026 is wrong.
...
- **What it is:** An emerging spec (multiple active proposals) for running MCP-style tool calls in browser JavaScript contexts, so that in-page AI assistants can discover and invoke tools exposed by the webpage itself. The W3C Web Machine Learning Community Group is incubating a formal spec at `webmachinelearning.github.io/webmcp/` ([source](https://webmachinelearning.github.io/webmcp/)). A separate MCP-B implementation extends MCP specifically for browser tab contexts ([source](https://github.com/MiguelsPizza/WebMCP)). The two proposals are not interoperable.
...
- **What's hype:** Two distinct specifications share the name WebMCP — the W3C draft and the MCP-B project. They are not interoperable. The fragmentation makes production adoption premature. For production code, MCP (server-side) is the current standard; WebMCP (browser-side) is wait-and-see until the W3C spec reaches Candidate Recommendation. "WebMCP will replace MCP" is a wrong framing — they're complementary surfaces for different deployment contexts.
...
- **What it is:** Google's open protocol for inter-agent communication, announced April 2025 at Google Cloud Next ([source](https://developers.googleblog.com/en/a2a-a-new-era-of-agent-interoperability/)), donated to the Linux Foundation June 2025. Designed for letting one agent delegate tasks to another across vendor boundaries, with auth, state, and result handoff. Uses HTTP, Server-Sent Events, and JSON-RPC 2.0. Version 0.3 shipped August 2025.
...
- **What's hype:** Google positions A2A as a proposed standard for agent interop. It is one proposal among several — IBM's Agent Communication Protocol (ACP, [source](https://agentcommunicationprotocol.dev/introduction/welcome)) is a competing spec also under Linux Foundation governance. No protocol has been ratified by a standards body or adopted across major LLM vendors as of mid-2026. Most production multi-agent systems run single-vendor stacks where cross-vendor delegation is not a problem. "150+ supporters" reflects enterprise interest and Google's ecosystem influence; it does not indicate production adoption at scale.
...
- **What it is:** Andrej Karpathy's term (February 2025) for fully committing to what the model produces — operating on feel, forgetting the code even exists, not debugging or reading code directly. "You fully give in to the vibes, embrace exponentials, and forget that the code even exists." ([source](https://x.com/karpathy/status/1886192184808149383))
...
- **What's hype:** Useful cultural shorthand for a real workflow. As an engineering principle for production systems, vibe coding does not address verification, testing, or maintenance — those concerns sit outside the term's scope. The risk is not reading code means not catching mistakes. The mitigation is transparency (Claude Code surfaces what it's doing), not the vibe itself. Note: X.com blocks automated URL verification; the Karpathy tweet is well-documented via public record if the direct link fails.
...
- OpenAI Developer Community — *Plugin store and new chats with plugins — closed March 19, 2024* (ChatGPT Plugins wind-down announcement, April 9 shutdown date): https://community.openai.com/t/plugin-store-and-new-chats-with-plugins-closed-march-19-2024/689877
